...
description: "Songwriting craft, AI music generation prompts (Suno, HeartMuLa), parody, and lyrics."
...
## 10. HeartMuLa (Open-Source Suno Alternative)

[HeartMuLa](https://github.com/HeartMuLa/heartlib) generates full songs from lyrics + tags locally. Apache-2.0 licensed. Requires 8GB+ VRAM (GPU) or runs very slowly on CPU.

**Input format:** Same as Suno — structural tags (`[Verse]`, `[Chorus]`) in lyrics, comma-separated style tags. See `references/heartmula.md` for installation, patches, and full usage.

**When to choose HeartMuLa over Suno:** offline/private generation, no subscription, reproducible results, custom fine-tuning. **When to choose Suno:** zero setup, faster iteration, better vocal quality out of the box.
