...
| **Text layout (Pretext)** | Prose / text | DOM-free text reflow, text-as-geometry, kinetic type | `references/pretext-patterns.md` |

### Text Layout with @chenglou/pretext

For demos where **text itself is the visual medium** — flowing around obstacles, shattering into particles, word-brick games, editorial multi-column — use [`@chenglou/pretext`](https://github.com/chenglou/pretext) (15KB, zero-dep). It measures line breaks and per-grapheme positions via canvas, enabling text reflow at 60fps without DOM reflow.

**When to use pretext instead of pure p5.js:**
- Text flowing around a moving shape (the viral demo pattern)
- Games where level geometry is made of real words
- Shatter/particle effects on individual letters
- Multiline shrink-wrap (smallest container that fits text)
- Kinetic typography with per-glyph physics

**Key API:**
```js
import { prepareWithSegments, layoutWithLines, layoutNextLineRange, materializeLineRange } from "https://esm.sh/@chenglou/pretext@0.0.6";
```

**Templates:** `templates/pretext-hello-orb-flow.html` (text reflowing around moving orb), `templates/pretext-donut-orbit.html` (ASCII mask obstacles, draggable shapes, selectable text).

**Patterns:** `references/pretext-patterns.md` — flow-around-obstacle, text-as-geometry game, shatter/explode, ASCII mask, editorial multi-column, shrink-wrap, kinetic type.

**Critical:** Pin `@0.0.6`. Call `prepareWithSegments()` once per text+font pair (expensive), then only `layoutNextLineRange()` per frame (cheap). Match `ctx.font` string exactly to CSS.
