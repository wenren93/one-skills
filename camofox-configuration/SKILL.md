---
name: camofox-configuration
description: "Configure Camofox anti-detection browser with Hermes Agent — environment variables, config.yaml, Docker setup, persistence, and multi-user tab adoption."
version: 1.0.0
author: agent
license: MIT
...
metadata:
  hermes:
    tags: [browser, camofox, anti-detection, fingerprint, docker, automation]
...
**Reference:** `references/install-and-cli.md` — CLI commands, server endpoints, Docker networking, Makefile targets.
...
```bash
# Default container (auto-detects arch)
cd camofox-browser && make up

# Custom container with persistence + VNC
mkdir -p ~/.camofox-docker
docker run -d \
  --name camofox-browser \
  --restart unless-stopped \
...
# VNC live view at http://localhost:6080 (noVNC)
# Native VNC client: localhost:5901

# Stop/remove default container before custom one
make down
```
...
```bash
# Check config
hermes config | grep -A 15 "browser:"

# Check env vars
cat ~/.hermes/.env | grep CAMOFOX

# Test browser tools
hermes chat -q "Open example.com and take a screenshot"
```
...
# Look for: "health probe: no successful navigation in 2+ minutes"
```

**Fixes (try in order):**
1. **Kill and let Hermes restart the process:**
   ```bash
   pkill -f camofox-browser
   # Hermes auto-restarts it; wait 3-5 seconds, then retry browser_navigate
   ```
...
4. **Switch engine as fallback:**
   ```bash
   hermes config set browser.engine auto
   # or explicitly: hermes config set browser.engine playwright
   ```

### Chrome Headless Fallback (macOS)

When Camofox is completely broken and you need a screenshot NOW, Chrome's built-in headless mode works without any dependencies:
...
- For bot-protected sites, use Playwright with anti-detection measures instead (see `camofox-setup` skill → `references/playwright-anti-detection.md`).
- Stderr will show harmless warnings (allocator, sandbox, GCM errors) — ignore them. The screenshot is written successfully.
- Combine with `vision_analyze` to inspect the result: `vision_analyze(image_url="/tmp/screenshot.png")`
...
8. **`.env` changes are NOT live** — The browser tool reads `CAMOFOX_URL` at session start. Commenting it out or changing it mid-session does NOT redirect the browser tool away from Camofox. You must start a **new Hermes session** (or restart the app) for `.env` changes to take effect. This is why changing `browser.engine` alone doesn't help mid-session.
9. **Install/cache issues** — Binary missing, cache cleanup loops, SIGKILL from Gatekeeper, and launchd configuration are covered in the Installation section above and in `references/camofox-setup-notes.md`.
