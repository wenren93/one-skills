...
description: "Hermes Kanban multi-agent board: orchestrator decomposition playbook + worker pitfalls, handoff patterns, and edge cases."
version: 3.0.0
...
> For workers dispatched by the kanban board. The **lifecycle** (6 steps: orient → work → heartbeat → block/complete) is auto-injected via the `KANBAN_GUIDANCE` system-prompt block. This section covers deeper detail on handoff shapes, retry diagnostics, and edge cases.
...
**Coding task needing human review:** Block instead of complete with `reason` prefixed `review-required:`. Drop structured metadata into a comment first since `kanban_block` only carries the human-readable reason.

```python
kanban_comment(body="review-required handoff:\n" + json.dumps({
    "changed_files": [...], "tests_run": 14, "tests_passed": 14,
    "diff_path": "/path/to/worktree", "decisions": [...],
}, indent=2))
kanban_block(reason="review-required: rate limiter shipped, 14/14 tests pass — needs eyes on the user_id/IP fallback choice")
```
...
Pass created card ids in `created_cards` on `kanban_complete`. The kernel verifies each id exists and was created by your profile. **Only list ids you captured from successful `kanban_create` return values — never invent ids.**
...
If `kanban_show` returns `runs: [...]` with closed runs, you're a retry:
...
- Don't call `clarify` — you're headless. Use `kanban_comment` + `kanban_block` instead.
...
Every tool has a CLI equivalent: `kanban_show` ↔ `hermes kanban show <id> --json`, `kanban_complete` ↔ `hermes kanban complete <id> --summary "..."`, etc. Use the tools from inside an agent; the CLI exists for the human at the terminal.
