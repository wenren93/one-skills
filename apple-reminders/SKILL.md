...
## User Preferences and Pitfalls

### Platform Preference
- User **闻人** prefers receiving reminders via the current chat platform (WeChat) rather than Apple Reminders.
- When user says "设置一个消息提醒" (set a message reminder), they mean a chat platform reminder, not Apple Reminders.
- For time-sensitive reminders (like "today at 11am"), use `cronjob` tool with `deliver: 'origin'` to send to current chat.
- Only use Apple Reminders when user explicitly mentions "Apple提醒", "Reminders app", or wants cross-device sync.

### Time Confirmation Pitfall
- **Always verify current time** before making time-related statements or calculations.
- Use `date` command to get accurate current time; do not estimate or assume.
- Example mistake: Said "现在已经是晚上7点半了" when actual time was 09:24 AM.
- When user asks "现在几点", respond with exact time from `date` command, not estimates.

### Reminder Workflow
1. For immediate/soon reminders (today): Use `cronjob` with `deliver: 'origin'`
2. For future/long-term reminders: Use Apple Reminders (`remindctl`)
3. When user wants to delete one after setting both: Offer to delete the Apple Reminders version
