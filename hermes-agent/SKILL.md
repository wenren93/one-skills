...
Camofox is an anti-detection browser backend built on Camoufox (Firefox fork with C++ engine-level fingerprint spoofing). Use when standard browser tools get blocked by bot detection, or when you need persistent browser sessions with login state.
...
browser:
  ...
  camofox:
    ...
    user_id: ""                 # External userId for multi-user tab management
    adopt_existing_tab: false   # Reuse existing tab
    rewrite_loopback_urls: false  # Docker loopback rewriting
```

**Full documentation:** `skill_view(name="hermes-agent", file_path="references/browser-camofox-config.md")`

**Additional references:**
- `references/camofox-install-cli.md` — CLI commands, server endpoints, Docker networking
- `references/camofox-setup-notes.md` — Firefox binary download, macOS launchd autostart, emergency fallbacks
- `references/camofox-playwright-anti-detection.md` — Playwright anti-detection patterns
- `references/camofox-macos-launchd.md` — macOS launchd plist for background service
...
### Time-sensitive queries (date boundaries, timezone conversion)

When a user asks about "today's" events near midnight (±2 hours of 00:00), the date may have already changed. **Always run `date` via terminal** before stating or implying the current time — the system context header shows conversation start date but NOT the current clock time.

Key pitfalls:
- **Never fabricate the current time.** Saying "it's already 7:30 PM" when it's 9:24 AM erodes trust. Always check with `date`.
- **Search with explicit dates**, not "today" — search engines misinterpret "today" across timezones. Use `YYYY-MM-DD` or `M月D日` format.
- **Cross-site timezone date-shift:** English-language sports sites (ESPN, etc.) display dates in US Eastern. A match listed as "July 11" at 5 PM ET is actually **July 12 at 5 AM Beijing time**. Always convert BOTH time AND date. Formula: if ET hour + 12 ≥ 24, Beijing date = ET date + 1.
- **Prefer local-timezone sources** for the user's region (e.g., fifawatch.com/zh/ for Beijing-timezone users) — they already show correct dates and times.
...
### opencli / feed cronjob issues

When opencli commands fail (CSS selectors, login, cronjob model drift, WeChat rate limiting), see `references/opencli-troubleshooting.md`.
...
### Authoring Skills (User-Local)

Skills live at `~/.hermes/skills/<maybe-category>/<name>/SKILL.md`. Create via `skill_manage(action='create')`. For in-repo skills (committed, shipped with the package), use `write_file` to `skills/<category>/<name>/SKILL.md`.

**Required frontmatter:** `name` (≤64 chars, lowercase+hyphens), `description` (≤1024 chars, start with "Use when..."), body after closing `---`.
...
**Size:** Aim for 8-15k chars. If pushing past 20k, split into `references/*.md`.
