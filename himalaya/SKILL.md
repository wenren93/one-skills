...
An alternative email CLI for QQ Mail / Tencent enterprise email. Install via `npm install -g @tencent-qqmail/agently-cli`.

### Setup

```bash
npx skills add https://agent.qq.com --skill -g -y
agently-cli auth login    # Interactive — run with background+pty, extract auth URL
agently-cli +me           # Verify
```

### Common Operations

```bash
agently-cli message +list --limit 10           # List emails
agently-cli message +list --is-unread --limit 10  # Unread only
agently-cli message +read --id <message_id>    # Read specific email
agently-cli message +send --to <email> --subject <subject> --body <body>
```

### WeChat Notification Integration

For email monitoring with WeChat delivery, create a cron script that checks `agently-cli message +list --is-unread` and delivers new email summaries via WeChat. State tracking via JSON file prevents duplicate notifications. Monitor at 15-minute intervals.
